...
export async function fetchAllStudentsAverageProgressWithDetails() {
    ...
    try {
        ...
        const latestProgressEntries = await prisma.$queryRaw<
            { studentId: string; courseInstanceId: string; progress: number; firebase_uid: string; email: string; first_name: string; last_name: string }[]
        >`
        ...
        const studentProgressMap: Record<string, { firebaseUid: string; email: string; firstName: string; lastName: string; progressList: number[] }> = {};

        latestProgressEntries.forEach(({ studentId, progress, firebase_uid, email, first_name, last_name }) => {
            if (!studentProgressMap[studentId]) {
                studentProgressMap[studentId] = { firebaseUid: firebase_uid, email, firstName: first_name, lastName: last_name, progressList: [] };
            }
            studentProgressMap[studentId].progressList.push(progress);
        });

        // Compute the average progress for each student
        const studentAverages = Object.entries(studentProgressMap).map(([studentId, { firebaseUid, email, firstName, lastName, progressList }]) => ({
            studentId,
            firebaseUid,
            email,
            firstName,
