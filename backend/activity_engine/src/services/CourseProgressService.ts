...
export class CourseProgressService {
  ...
  public async updateSectionItemProgress(
  ...
  ): Promise<UpdatedEntities[]> {  // Now returns an array of updates
    ...
    for (const sectionItemId of sectionItemIds) {
      try {
        ...
        if (cascade) {
          const sectionItemDetails = await courseProgressRepo.getSectionItemDetails(
            courseInstanceId,
            studentId,
            sectionItemId
          );
          ...
          if (!sectionItemDetails) {
            throw new Error(`Section item details not found for sectionItemId ${sectionItemId}`);
          }
          ...
          if (nextSectionItemId) {
            ...
            updates.push(updateEntityListFields(updatedEntities, {
      ...
      if (cascade) {
        if (nextSectionId) {
          ...
          if (nextSectionDetails) {
            const { sectionItemId: nextSectionFirstSectionItemId } = nextSectionDetails;
            ...
            throw new Error(`Section details not found for sectionId ${nextSectionId}`);
          }
        } else {
      ...
      if (cascade) {
        if (nextModuleId) {
          ...
          if (!nextModuleDetails) {
            throw new Error(`Module details not found for moduleId ${nextModuleId}`);
          }
          ...
          if (!nextSectionDetails) {
            throw new Error(`Section details not found for sectionId ${nextModuleFirstSectionId}`);
          }

          const { sectionItemId: nextModuleFirstSectionItemId } = nextSectionDetails;
  ...
  */
  ...
  public async getSectionItemProgress(courseInstanceId: string, studentId: string, sectionItemId: string) {
    console.log("dcbnbdscmdsvmnbvnmfv courseInstanceId:", courseInstanceId, "studentId:", studentId, "sectionItemId:", sectionItemId);
