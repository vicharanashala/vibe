...
describe('AttemptController', () => {
  ...
  describe('POST /quizzes/:quizId/attempt/:attemptId/submit', () => {
    it('should submit answers for all question types in a single quiz', async () => {
      ...
      const solCorrectLotItem = solDetails.body.lotItems.find(
        (item: any) => item.text.includes(solNameValue)
      );
      ...
      const smlParamValues = [
        ...Object.values(smlDetails.body.parameterMap.animal ? { animal: smlDetails.body.parameterMap.animal } : {}),
        ...Object.values(smlDetails.body.parameterMap.color ? { color: smlDetails.body.parameterMap.color } : {}),
      ];
      ...
      const otlOrders = Object.entries(otlStepMap).map(([stepNum, stepValue]) => {
        const lotItem = otlDetails.body.lotItems.find(
          (item: any) => item.text.includes(`Step ${stepNum}:`) && item.text.includes(stepValue)
        );
        return {
      ...
      const submitRes = await request(app)
        ...
        .send({
          answers: [
            ...
            // SELECT_MANY_IN_LOT
            { questionId: smlId, questionType: 'SELECT_MANY_IN_LOT', answer: { lotItemIds: smlCorrectLotItemIds } },
            // ORDER_THE_LOTS
            { questionId: otlId, questionType: 'ORDER_THE_LOTS', answer: { orders: otlOrders } },
            // DESCRIPTIVE
            { questionId: desId, questionType: 'DESCRIPTIVE', answer: { answerText: 'Compiling is the process...' } },
          ],
        });
      expect(submitRes.status).toBe(200);
      expect(submitRes.body).toHaveProperty('totalScore');
