...
import { PacingService } from '#users/services/PacingService.js';
import { PacingPlanResponse, CoursePacingOverviewResponse } from '#users/classes/validators/PacingValidators.js';
...
export class EnrollmentController {
  constructor(
    ...
    @inject(USERS_TYPES.PacingService)
  ...
  async unenrollUser(
    ...
    setAuditTrail(req, {
      category: AuditCategory.ENROLLMENT,
      action:
        enrollmentData.role === 'INSTRUCTOR'
          ? AuditAction.ENROLLMENT_REMOVE_INSTRUCTOR
          : AuditAction.ENROLLMENT_REMOVE_STUDENT,
      actor: {
        id: ObjectId.createFromHexString(user._id.toString()),
        name: `${user.firstName} ${user.lastName}`,
        email: user.email,
  ...
  @OpenAPI({
    ...
    description:
      'Updates the status (ACTIVE/INACTIVE) of a specific enrollment.',
  })
  @Authorized()
  ...
  async changeStatus(
    ...
    setAuditTrail(req, {
      ...
      action:
        status === 'ACTIVE'
          ? AuditAction.ENROLLMENT_ENABLE
          : AuditAction.ENROLLMENT_DISABLE,
      actor: {
        id: ObjectId.createFromHexString(user._id.toString()),
        name: `${user.firstName} ${user.lastName}`,
        email: user.email,
  ...
  @OpenAPI({
    ...
    description:
      'Updates the status (ACTIVE/INACTIVE) of multiple enrollments in a course version.',
  })
  @Authorized()
  ...
  async bulkChangeStatus(
    ...
    setAuditTrail(req, {
      ...
      action:
        status === 'ACTIVE'
          ? AuditAction.BULK_ENROLLMENT_ENABLE
          : AuditAction.BULK_ENROLLMENT_DISABLE,
      actor: {
        id: ObjectId.createFromHexString(user._id.toString()),
        name: `${user.firstName} ${user.lastName}`,
        email: user.email,
  ...
  @OpenAPI({
    ...
    description:
      'Returns content summary (completion %, total items, quiz score, item type counts) for a specific enrolled student. Intended to be called only when the instructor opens the View Progress modal.',
  })
  @Authorized()
  @Get(
    '/:userId/enrollments/courses/:courseId/versions/:versionId/progress-detail',
  )
  @HttpCode(200)
  ...
  @OpenAPI({
    ...
    description:
      'Returns the full course module/section structure and the current learning position for a specific enrolled student. Intended to be called lazily when the instructor clicks View Course Structure.',
  })
  @Authorized()
  @Get(
    '/:userId/enrollments/courses/:courseId/versions/:versionId/course-structure',
  )
  @HttpCode(200)
  ...
  @OpenAPI({ summary: 'Get pacing overview for all students in a course version' })
  @Authorized()
  @Get('/enrollments/courses/:courseId/versions/:versionId/pacing-overview')
  @HttpCode(200)
  @ResponseSchema(CoursePacingOverviewResponse, { description: 'Aggregated pacing overview' })
  async getCoursePacingOverview(
    @Param('courseId') courseId: string,
    @Param('versionId') versionId: string,
    @Ability(getEnrollmentAbility) { ability },
  ): Promise<CoursePacingOverviewResponse> {
    const enrollmentResource = subject('Enrollment', { courseId, versionId });

    if (!ability.can(EnrollmentActions.ViewAll, enrollmentResource)) {
      throw new ForbiddenError(
        'You do not have permission to view pacing overview for this course',
      );
    }

    return await this.pacingService.getCoursePacingOverview(courseId, versionId);
  }

  @OpenAPI({ summary: "Get a specific student's pacing plan (instructor view)" })
  @Authorized()
  @Get('/enrollments/courses/:courseId/versions/:versionId/students/:userId/pacing')
  @HttpCode(200)
  @ResponseSchema(PacingPlanResponse, { description: "Student's pacing plan" })
  async getStudentPacingDetail(
    @Param('courseId') courseId: string,
    @Param('versionId') versionId: string,
    @Param('userId') userId: string,
    @QueryParam('cohortId') cohortId: string | undefined,
    @QueryParam('useTeacherDeadline') useTeacherDeadline: boolean | undefined,
    @Ability(getEnrollmentAbility) { ability },
    ...
    if (!ability.can(EnrollmentActions.ViewAll, enrollmentResource)) {
      throw new ForbiddenError(
        'You do not have permission to view student pacing details for this course',
      );
    }

    const forceDeadline = useTeacherDeadline !== undefined ? Boolean(useTeacherDeadline) : true;
    return (await this.pacingService.getPacingPlan(userId, courseId, versionId, cohortId, forceDeadline)) as any;
  }
  // @Authorized()
  ...
  @Authorized()
  @Get(
    '/:userId/enrollments/courses/:courseId/versions/:versionId/modules/progress',
  )
  @HttpCode(200)
  ...
  async getUserModuleProgress(
    ...
    const moduleProgress =
      await this.enrollmentService.getModuleProgressForUser(
        userId,
        courseId,
        versionId,
        await this.scopedStudentCohort(
  ...
  @Authorized()
  @Post(
    '/enrollments/courses/:courseId/versions/:courseVersionId/cohorts/:cohortId/policy-acknowledge',
  )
  @HttpCode(200)
  async acknowledgePolicyUpdate(
    @Param('courseId') courseId: string,
    @Param('courseVersionId') courseVersionId: string,
    @Param('cohortId') cohortId: string,
    @CurrentUser() user: any,
  ) {
    await this.enrollmentService.clearPolicyReacknowledgement(
      user._id.toString(),
      courseId,
      courseVersionId,
      cohortId,
