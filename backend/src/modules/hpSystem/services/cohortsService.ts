...
} from '../classes/validators/courseAndCohorts.js';
...
export class CohortsService extends BaseService {
  ...
  async listCourseVersions(
    userId: string,
    query: CourseVersionListQueryDto,
  ): Promise<CourseVersionListResponseDto> {
    return await this._withTransaction(async (session: ClientSession) => {
      const [hardcodedCourses, dynamicCourses, instructorEnrollments] =
        await Promise.all([
      ...
      const enrolledVersionIds = new Set(
        instructorEnrollments.map(e => e.courseVersionId),
      );

      const hardcodedMappings = [
        {
          pseudoVersionId: '000000000000000000000001',
          cohortVersionId: '6968e12cbf2860d6e39051af',
        },
        {
          pseudoVersionId: '000000000000000000000001',
          cohortVersionId: '6970f87e30644cbc74b67150',
        },
        {
          pseudoVersionId: '000000000000000000000001',
          cohortVersionId: '697b4e262942654879011c57',
        },
        {
          pseudoVersionId: '000000000000000000000002',
          cohortVersionId: '69903415e1930c015760a719',
        },
        {
          pseudoVersionId: '000000000000000000000002',
          cohortVersionId: '69942dc6d6d99b252e3a54ff',
        },
      ];
      ...
      const allCourses = allCoursesRaw
        .map(course => {
          const filteredVersions = course.versions.filter(
            v =>
              enrolledVersionIds.has(v.courseVersionId) ||
              enrolledPseudoIds.has(v.courseVersionId),
          );
      ...
      const totalVersions = allCourses.reduce(
        (sum, course) => sum + course.versions.length,
        0,
      );
  ...
  private async _handleExisitingCohorts(
    versionId: string,
  ): Promise<CohortListItemDto[] | null> {
    if (
      versionId !== '000000000000000000000001' &&
      versionId !== '000000000000000000000002'
    )
      return null;
    ...
    if (versionId === '000000000000000000000001') {
      ...
      const cohorts = [
        ...
        {
          cohortName: 'Dijkstrians',
          cohortVersionId: '6970f87e30644cbc74b67150',
        },
        {
          cohortName: 'Kruskalians',
          cohortVersionId: '697b4e262942654879011c57',
        },
      ] as const;
      ...
      const results = await Promise.all(
        cohorts.map(async c => {
          const [
            ...
            this.cohortRepository.getTotalStudentsCountForCourseVersion(
              c.cohortVersionId,
            ),
            this.activityRepository.getCountByCohortName(
              c.cohortName,
              versionId,
            ),
            this.activityRepository.getDraftCountByCohortName(
              c.cohortName,
              versionId,
            ),
            this.activityRepository.getPublishedCountByCohortName(
              c.cohortName,
              versionId,
            ),
            this.cohortRepository.getTotalHpDistributedByCohort(
              c.cohortVersionId,
            ),
          ]);
  ...
  async listCohorts(
    userId: string,
    query: CohortListQueryDto,
  ): Promise<CohortListResponseDto> {
    return await this._withTransaction(async (session: ClientSession) => {
      let cohorts: CohortListItemDto[] = [];

      const instructorEnrollments =
        await this.cohortRepository.getInstructorActiveEnrollments(userId);
      const enrolledVersionIds = new Set(
        instructorEnrollments.map(e => e.courseVersionId),
      );
      const enrolledCohortIds = new Set(
        instructorEnrollments.map(e => e.cohortId).filter(Boolean),
      );

      let courseVersionName = '';

      if (query.courseVersionId) {
        const isGeneralInstructorForVersion = instructorEnrollments.some(
          e => e.courseVersionId === query.courseVersionId && !e.cohortId,
        );
        ...
        const fetched = await this._handleExisitingCohorts(
        ...
        if (fetched) {
          ...
          const filteredFetched = fetched.filter(c => {
            const mapping = hardcodedMappings.find(
              m => m.cohortName === c.cohortName,
            );
        ...
        if (!isGeneralInstructorForVersion) {
          const dbCohortsData =
            await this.cohortRepository.getCohortsByVersionId(
              query.courseVersionId,
            );
          dbCohorts = dbCohorts.filter(c => {
            const matchingData = dbCohortsData.find(
              dbC => dbC.name === c.cohortName,
            );
            return (
              matchingData &&
              enrolledCohortIds.has(matchingData._id?.toString() || '')
            );
          });
        }
      ...
      if (!courseVersionName)
        courseVersionName =
          await this.cohortRepository.getCourseVersionNameById(
            query.courseVersionId,
          );
   ...
   */
  private async _fetchDbCohorts(
    courseVersionId: string,
    courseId: string = '',
    isPublic?: boolean,
  ): Promise<CohortListItemDto[]> {
    const dbCohorts = await this.cohortRepository.getCohortsByVersionId(
      courseVersionId,
      isPublic,
    );
    ...
    const results = await Promise.all(
      dbCohorts.map(async cohort => {
        ...
        const [
          ...
          this.cohortRepository.getTotalStudentsCountForCohort(
            courseVersionId,
            cohortId,
          ),
          this.cohortRepository.getTotalHpDistributedByCohort(
        ...
        return {
          ...
          lastActivityAt:
            cohort.updatedAt?.toISOString?.() ?? new Date().toISOString(),
          createdAt:
            cohort.createdAt?.toISOString?.() ?? new Date().toISOString(),
  ...
  private async _handleExisitingCohortStudents(
    ...
    return this.cohortRepository.getStudentsForExistingCohortByVersionId(
      actualVersionId,
      query,
    );
  }
  ...
  async listCohortStudents(input: {
    ...
    return await this._withTransaction(async (session: ClientSession) => {
      ...
      let students = await this._handleExisitingCohortStudents(
        versionId,
        cohortName,
        query,
      );
      ...
      if (!students) {
        const dbCohorts =
          await this.cohortRepository.getCohortsByVersionId(versionId);
  ...
  async listStudentCohorts(
    ...
    return await this._withTransaction(async (session: ClientSession) => {
      ...
      for (const mapping of hardcodedMappings) {
        ...
        const isEnrolled =
          await this.cohortRepository.isStudentEnrolledInVersion(
            userId,
            mapping.cohortVersionId,
          );
        if (isEnrolled) {
          // Fetch the hardcoded cohort data using the pseudo version ID
          const hardcodedList = await this._handleExisitingCohorts(
            mapping.pseudoVersionId,
          );
          if (hardcodedList) {
            const matchedCohort = hardcodedList.find(
              c => c.cohortName === mapping.cohortName,
            );
            if (matchedCohort) {
      ...
      const enrollments =
        await this.cohortRepository.getStudentActiveEnrollments(userId);
      if (enrollments && enrollments.length > 0) {
        const versionIds = [
          ...new Set(enrollments.map(e => e.courseVersionId)),
        ];

        // Exclude hardcoded version ids from dynamic lookup to avoid double counting or unnecessary db processing
        const dynamicVersionIds = versionIds.filter(
          id => !hardcodedMappings.some(m => m.cohortVersionId === id),
        );
        ...
        if (dynamicVersionIds.length > 0) {
          ...
          const hpEnrollments = enrollments.filter(e =>
            hpEnabledVersionIds.has(e.courseVersionId),
          );
          ...
          for (const [versionId, versionEnrollments] of enrollmentsByVersion) {
            ...
            const dynamic = await this._fetchDbCohorts(
              versionId,
              courseId,
              true,
            );
            ...
            if (hasNoCohortName) {
              ...
              const matched = dynamic.filter(c =>
                enrolledCohortNames.has(c.cohortName.toLowerCase()),
              );
      ...
      const [completedActivitiesCounts, totalHp] = await Promise.all([
        this.activitySubmissionsRepository.getCompletedActivitiesCountByStudentId(
          userId,
        ),
      ...
      const finalCohorts = studentCohorts.map(cohort => {
        ...
        const match = completedActivitiesCounts.find(
          c => c.cohort?.toLowerCase() === cohort.cohortName?.toLowerCase(),
        );
        if (match) {
        ...
        if (publishedActivities > 0) {
          percentCompleted = Number(
            ((completedCount / publishedActivities) * 100).toFixed(2),
          );
        }
        return {
          ...cohort,
