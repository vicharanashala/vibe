...
export class CohortsController {
  ...
  async listCourseVersions(
    @QueryParams() query: CourseVersionListQueryDto,
  ...
  async listCohorts(
    @QueryParams() query: CohortListQueryDto,
    @CurrentUser() user: IUser,
  ) {
    const userId = user._id.toString();
  ...
  @OpenAPI({
    summary: 'List cohorts the student is enrolled in (hpSystem enabled)',
  })
  @Get('/student-cohorts')
  @Authorized()
  @HttpCode(200)
  ...
  async listCohortStudents(
    @Param('versionId') versionId: string,
    @Param('cohortName') cohortName: string,
    @QueryParams() query: CohortStudentsListQueryDto,
    ...
    return await this.cohortsService.listCohortStudents({
      versionId,
      cohortName,
