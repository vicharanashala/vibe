...
} from '#root/modules/hpSystem/classes/validators/courseAndCohorts.js';
...
export class CohortRepository implements ICohortRepository {
  ...
  private async init() {
    ...
    this.courseSettingsCollection =
      await this.db.getCollection<any>('courseSettings');
    ...
    this.hpLedgerCollection =
      await this.db.getCollection<HpLedger>('hp_ledger');
  ...
  async getTotalStudentsCountForCourseVersion(
  ...
  async getTotalStudentsCountForCohort(
    courseVersionId: string,
    cohortId: string,
  ): Promise<number> {
    await this.init();
  ...
  async getStudentsForCohortByCohortId(
    ...
    const pipeline: any[] = [
      {
        $match: {
          $and: [{$or: orVersionMatch}, {$or: orCohortMatch}],
          isDeleted: {$ne: true},
        },
      },
  ...
  private async _getCohortMatchConditions(
    cohort: string,
    versionId: string,
  ): Promise<any[]> {
    ...
    if (ObjectId.isValid(cohort)) {
      ...
      const dynamicCohort = await this.cohortsCollection.findOne({
        name: cohort,
   ...
   */
  async isStudentEnrolledInVersion(
    userId: string,
    courseVersionId: string,
  ): Promise<boolean> {
    await this.init();
    const userObjId = new ObjectId(userId);
    const versionObjId = ObjectId.isValid(courseVersionId)
  ...
  async getStudentTotalHpAcrossEnrollments(userId: string): Promise<number> {
    ...
    const result = await this.enrollmentCollection
      .aggregate([
        ...
        {
          $match: {
            $or: [{isOverride: true}, {'courseSettingsDoc.0': {$exists: true}}],
          },
        },
  ...
  async getDynamicCoursesWithVersions(
    session?: ClientSession,
    ...
    return await this.courseSettingsCollection
      .aggregate<CourseWithVersionsDto>(pipeline, {session})
      .toArray();
  }

  async getInstructorActiveEnrollments(
    userId: string,
  ): Promise<
    Array<{courseId: string; courseVersionId: string; cohortId?: string}>
  > {
    await this.init();
    const userObjId = new ObjectId(userId);
