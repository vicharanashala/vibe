...
export function useListStudentQuestionsBySegment(): {
  getQuestions: (
    courseId: string,
    courseVersionId: string,
    segmentId: string,
  ...
  const getQuestions = useCallback(async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch student questions: ${response.status}`);
      }

      return await response.json();
    } catch (err: any) {
      const message = err?.message || 'Failed to fetch student questions';
      setError(message);
      throw err;
    } finally {
      setLoading(false);
...
export function useUpdateStudentQuestionStatus(): {
  ...
  const updateStatus = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to update student question status: ${response.status}`);
      }
