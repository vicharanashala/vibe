...
// POST /peer-review-assessments  (Phase 2.2.4 — peer-review teacher create)
//
// The OpenAPI schema (frontend/src/types/schema.ts) is generated by
// `pnpm copy && pnpm gen-schema` from the backend's running OpenAPI
// spec. Until that regen is run, the `/peer-review-assessments`
// operation is not in `components['paths']`, so we use `as any` to
// call api.useMutation with the new path. After the regen, the cast
// can be removed and the response body can be typed strictly.
//
// Doc: /home/shreyas/phase1-implementation-peer-review.md §2.2.4
export function useCreatePeerReviewAssessment(): {
  ...
  // The openapi-react-query client is typed against `paths`. The new
  // route is not yet in `paths` (regen pending), so we cast at the
  // call site. Functionally identical to a regular mutation.
  const result = (api as any).useMutation('post', '/peer-review-assessments');
  return {
    ...result,
    error: result.error ? (result.error.message || 'Peer-review assessment create failed') : null,
  };
}
...
export function useUpdatePeerReviewAssessment(): {
  ...
  mutateAsync: (variables: { params: { path: { id: string } }, body: any }) => Promise<any>,
  data: any,
  error: string | null,
  isPending: boolean,
  isSuccess: boolean,
  ...
  const result = (api as any).useMutation('patch', '/peer-review-assessments/{id}');
  return {
    ...result,
    error: result.error ? (result.error.message || 'Peer-review assessment update failed') : null,
  };
}

// POST /peer-review-assessments/:id/close
export function useClosePeerReviewAssessment(): {
  mutate: (variables: { params: { path: { id: string } } }) => void,
  ...
  const result = (api as any).useMutation('post', '/peer-review-assessments/{id}/close');
  return {
    ...result,
