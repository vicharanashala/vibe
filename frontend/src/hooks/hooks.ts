...
// NOTE: this repo's OpenAPI generator (backend/scripts/generate-openapi.cjs)
// is currently a stub — it writes a hardcoded schema with only a fake
// /health route, regardless of what controllers actually exist. That means
// api.useQuery's strict path typing can't recognize ANY newer route,
// certificates included (the same is true for the existing
// ejection-policy-hooks.ts, which has identical PathsWithMethod errors).
// Until someone implements real schema generation for this repo, new
// routes have to bypass the typed wrapper and call fetchClient directly,
// with the path cast past the (currently incomplete) type check. This
// still goes through the same fetchClient as every other call — same auth
// header injection, same credentials handling — only the compile-time path
...
export function useMyCertificates(enabled: boolean = true): {
  data: MyCertificate[] | undefined,
  isLoading: boolean,
  error: string | null,
  refetch: () => void,
} {
  const [data, setData] = useState<MyCertificate[] | undefined>(undefined);
  ...
  const fetchCertificates = useCallback(async () => {
    ...
    setError(null);
    try {
      const {data: body, error: fetchError} = await fetchClient.GET(
        '/certificates/mine' as any,
      );
      if (fetchError) {
        throw new Error((fetchError as any)?.message || 'Failed to fetch certificates');
      }
      setData(body as unknown as MyCertificate[]);
    } catch (err: any) {
