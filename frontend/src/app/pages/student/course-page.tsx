...
import { AskBetalDrawer } from "./components/AskBetalDrawer";
...
export default function CoursePage() {
  ...
  const [isAskBetalOpen, setIsAskBetalOpen] = useState(false);
  ...
  useEffect(() => {
    if (isAskBetalOpen && currentItem?.type !== 'VIDEO') {
      setIsAskBetalOpen(false);
    }
  }, [currentItem, isAskBetalOpen]);
...
const [backgroundSectionInfo, setBackgroundSectionInfo] = useState<{
  ...
  const currentModule = useMemo(() => {
    const modulesList = (courseVersionData as any)?.modules || [];
    return modulesList.find((m: any) => m.moduleId === selectedModuleId || m._id === selectedModuleId);
  }, [courseVersionData, selectedModuleId]);

  const currentSection = useMemo(() => {
    return currentModule?.sections?.find((s: any) => s.sectionId === selectedSectionId || s._id === selectedSectionId);
                ...
                <div className="flex items-center gap-2 ml-auto">
                  {currentItem?.type === 'VIDEO' && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setIsAskBetalOpen(true)}
                      className="flex items-center gap-1.5 h-9 rounded-lg border-border/40 bg-card hover:bg-accent hover:text-accent-foreground shadow-xs font-semibold"
                    >
                      <Bot className="size-4 text-primary" />
                      <span className="hidden sm:inline">Ask Betal</span>
                    </Button>
                  )}
                  <ThemeToggle />
                </div>
  ...
  >
    ...
    <ChevronRight className="h-4 w-4" />
      ...
      <AskBetalDrawer
        ...
        currentVideoTitle={currentItem?.name}
        currentVideoId={currentItem?._id}
        open={isAskBetalOpen}
        onOpenChange={setIsAskBetalOpen}
      />
    </>
  );
};