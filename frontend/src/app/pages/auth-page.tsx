...
export default function AuthPage() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (result._tokenResponse.isNewUser) {
        ...
        await fetch(`${import.meta.env.VITE_BASE_URL}/auth/signup/google/`, {
          method: 'POST',
          headers: {
      ...
      setFormErrors({ ...formErrors, auth: "Failed to sign in with Google. Please try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEmailLogin = async () => {
    try {
      ...
      setFormErrors({ ...formErrors, auth: "Invalid email or password. Please try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEmailSignup = async () => {
    ...
    if (passwordStrength.value < 50) {
      setFormErrors({ ...formErrors, password: "Please create a stronger password" });
      return;
    }
    try {
      setLoading(true);
      ...
      if (isSignUpError) {
        ...
        setFormErrors({
          ...
          fullName: (Object.values(signupError?.errors?.find((e: any) => e.property === 'firstName')?.constraints || {}).join(', ') + (Object.values(signupError?.errors?.find((e: any) => e.property === 'lastName')?.constraints || {}).join(', '))).trim() || "",
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      if (user.role === 'teacher') navigate({ to: '/teacher' });
                      ...
                      <>
                        ...
                        <Card className="relative overflow-hidden bg-[rgb(240,248,250)] border-0 rounded-[8px] pt-[75px] pb-[34px]">
                          {!isSignUp ? (
                              ...
                              <CardContent className="space-y-10 md:px-20 px-5">
                                ...
                                <div className="relative my-6">
                                  <div className="relative flex justify-center text-xs uppercase">
                                    <span className="text-[rgb(52,152,169)] text-xl font-semibold">OR</span>
                                  </div>
                                </div>
                            ...
                            <div>
                              <CardHeader>
                                <CardTitle className="text-3xl md:text-4xl text-center text-[rgb(25,90,105)] font-bold leading-tight mb-10 md:mb-[60px]">
                                  Create {activeRole === 'student' ? 'Student' : 'Instructor'} Account
                                </CardTitle>
                              </CardHeader>
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="grid grid-cols-2 gap-1 text-xs text-[rgb(52,152,169)]">
                                        <div className="flex items-center gap-1">
                                          <Check className={cn("h-3 w-3", password.length >= 8 ? 'text-green-500' : 'text-[rgb(52,152,169)]')} />
                                          8+ characters
                                        </div>
                                        <div className="flex items-center gap-1">
                                          <Check className={cn("h-3 w-3", /[A-Z]/.test(password) ? 'text-green-500' : 'text-[rgb(52,152,169)]')} />
                                          Uppercase
                                        </div>
                                        <div className="flex items-center gap-1">
                                          <Check className={cn("h-3 w-3", /\d/.test(password) ? 'text-green-500' : 'text-[rgb(52,152,169)]')} />
                                          Numbers
                                        </div>
                                        <div className="flex items-center gap-1">
                                          <Check className={cn("h-3 w-3", /[!@#$%^&*(),.?":{}|<>]/.test(password) ? 'text-green-500' : 'text-[rgb(52,152,169)]')} />
                                          Special chars
                                        </div>
                                      </div>
                                    </div>
          ...
          <div className="w-full max-w-[1280px] mx-auto py-16 px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <p className="text-[#9CA3AF] text-base font-normal leading-6">Your journey starts here.</p>
              </div>
            ...
            <div className="bg-[rgb(240,248,250)] p-8 rounded-[8px] w-full max-w-[608px] ml-auto">
              <h4 className="text-[rgb(25,90,105)] text-base font-bold mb-4">Sign up for updates and insights.</h4>
