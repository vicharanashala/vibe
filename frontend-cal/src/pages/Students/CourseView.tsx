...
function useFetchCourseStatuses(courses: Course[]) {
  ...
  async function fetchCourseStatus(courseId: string): Promise<'IN_PROGRESS' | 'COMPLETED' | 'UNKNOWN'> {
...
const CourseView = () => {
  ...
  const { data: coursesData, error, isLoading } = useFetchCoursesWithAuthQuery();
  const courseStatuses = useFetchCourseStatuses(coursesData?.results || []);

  if (isLoading) return <p>Loading courses...</p>;
  if (error) {
    return <p>Error loading courses: {error.toString()}</p>;
  }
  ...
  return (
    <div className='p-4'>
      <div className='flex items-center justify-between'>
        ...
        <div className='mb-6'>
          <Select onValueChange={value => setFilter(value as 'All' | 'IN_PROGRESS' | 'COMPLETED')} defaultValue='IN_PROGRESS'>
          ...
          <Card key={course.course_id} className='w-full'>
            <CardHeader>
              <div className='flex items-center justify-between'>
                ...
                <div className={`flex w-20 justify-center rounded-sm p-1 text-xs text-white ${
                  courseStatuses[course.course_id] === 'IN_PROGRESS' ? 'bg-yellow-500' : 'bg-gray-500'
                }`}>
            ...
            <CardContent>
              <img src={course.image || 'https://excellentia.org.in/images/courses.jpg'} alt={course.name} className='h-40 w-full rounded object-cover' />
            </CardContent>
            <div className='p-4 text-center'>
              <Link to={`/module-view/${course.course_id}`} className='inline-block w-full rounded bg-gray-800 px-4 py-2 text-center text-white'>
                View
              </Link>
            </div>
          </Card>
